...
export const sendCreateComment = async (task, createCommentDto) => {
    console.log("--------------------")
    const url = API_BASE_URL + API_CONTEXT + "/workspaces/" + task.workspaceId + "/desks/" + task.deskId + "/tasks/" + task.id + "/comments";

    console.log(url, createCommentDto);
    ...
    if (!response.ok) {
        ...
        if (error.status === 401 && error.detail.includes('access')) {
            ...
            try {
                ...
                return await sendCreateComment(task, createCommentDto);
            } catch (in_error) {
